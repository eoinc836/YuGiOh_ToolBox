...
const startSearch = () => {
    let results = (performSearch(boxItems))
...
const sendSearch = async () => {
  ...
  try {
    console.log(boxItems,'box')
    // Make a POST request to the Django backend
    const response = await axios.post('http://localhost:8000/api_test/', boxItems, {
      headers: {
        'Content-Type': 'application/json',
        // If you need to send authentication token or other headers, include them here
      },
    });
